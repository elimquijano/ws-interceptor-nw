...
def parse_gps103_data(raw_data):
    ...
    if command == '001':
        ...
        data['type'] = 'send LCD/Handset, dispatch screen (notice) data'
    elif command == '171':
        ...
        data['type'] = 'tyres pressure monitoring'
    else:
        data['type'] = 'unknown'
    ...
    if data['type'] in ['location information', 'SOS alarm', 'low battery alarm', 'movement alarm', 'over speed alarm', 'geo-fence alarm', 'ACC alarm', 'accident alarm', 'shock sensor alarm', 'door alarm', 'oil leak/oil theft alarm', 'vehicle fault notification', 'vehicle maintenance notification']:
        data['data'] = {
            'imei': parts[0],
            'time': datetime.strptime(parts[2], '%y%m%d%H%M%S').strftime('%Y-%m-%d %H:%M:%S'),
            'gps_valid': parts[3],
    ...
    elif data['type'] in ['single track', 'track upon time interval', 'track upon distance interval', 'cancel auto track continuously', 'cancel alarm', 'set movement alarm', 'cancel movement alarm', 'set overspeed alarm', 'set time zone', 'cut off oil and power', 'resume oil and power', 'arm', 'disarm', 'switch to SMS mode', 'set geo-fence', 'cancel geo-fence', 'data load', 'cancel upload', 'activate less GPRS mode', 'deactivate less GPRS mode', 'automatic update positions of vehicle turns', 'set multi-area management', 'set IP, port for address function', 'activate speed limit mode', 'deactivate speed limit mode', 'activate speed limit']:
        data['data'] = {
            'imei': parts[0],
            'command': parts[1],
            'time': datetime.strptime(parts[2], '%y%m%d%H%M%S').strftime('%Y-%m-%d %H:%M:%S'),
        ...
        data['data'] = {
            ...
            'time': datetime.strptime(parts[3], '%y%m%d%H%M%S').strftime('%Y-%m-%d %H:%M:%S'),
    ...
    elif data['type'] == 'tyres pressure monitoring':
        data['data'] = {
            'imei': parts[0],
            'command': parts[1],
            'time': datetime.strptime(parts[2], '%y%m%d%H%M%S').strftime('%Y-%m-%d %H:%M:%S'),
