...
def parse_gps103_data(raw_data):
    ...
    for i, part in enumerate(parts):
        if (
            part == "F"
            and i + 2 < len(parts)
            and parts[i + 1].endswith(".000")
            and parts[i + 2] in ["A", "V"]
        ):
...
def parse_h02_data(raw_data):
    ...
    if data["type"] in [
        ...
        data["data"] = {"imei": parts[1], "num_list": parts[3]}
        ...
        data["data"] = {"imei": parts[1], "interval": parts[3]}
    elif data["type"] == "alarm setting":
        data["data"] = {"imei": parts[1], "key": parts[3], "type": parts[4]}
        ...
        data["data"] = {"imei": parts[1], "operation": parts[3]}
    ...
    # Devuelve el diccionario en lugar de JSON
    return data
...
def tcp_to_json(port, data):
    if port == 6001:
        ...
        data_dict = parse_h02_data(data)
        asyncio.run(
            broadcast(data_dict["data"]["imei"], data_dict["type"], data_dict["data"])
        )
